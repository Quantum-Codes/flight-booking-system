...
def userinput():
    ...
    if not OneFlight:  # Just Oneflight - returns False if it is a list of empty tuples
        ...
        for i in layer1:
            sql.execute(
                'SELECT to_city, id, departure, arrival,from_city FROM flights WHERE from_city = %s AND arrival>%s AND to_city = %s;',
                (i[0], i[2], dest)
            )
            layer2 = sql.fetchall()
        ...
        if TwoFlight:  # Just Twoflight - returns False if it is a list of empty tuples
            ...
            index = 0
            for i in layer1:
                sql.execute('SELECT to_city, id, departure, arrival FROM flights WHERE from_city = %s AND arrival>%s;',
                            (i[0], i[2]))  # A-B  B-C C-
                layer2 = sql.fetchall()
                for j in layer2:
                    sql.execute(
                        'SELECT id, arrival, departure, from_city,to_city FROM flights WHERE from_city = %s AND to_city = %s AND arrival>%s;',
                        (j[0], dest, j[2]))
                    layer3 = sql.fetchall()
                    for k in layer3:
  ...
  ''')
    ...
    if a == 1:
        ...
        for item in allflights:
            ...
            print(item[0], "    ", item[3], "    ", item[1], ' ' * (15 - len(item[1])), item[2], ' ' * (15 - len(item[2])),
                  str(item[5]), ' ' * (11 - len(str(item[5]))), str(item[4]), sep="")
    elif a == 2:
        flight = userinput()
        payment(flight, user)
